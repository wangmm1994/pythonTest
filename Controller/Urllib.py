...
class BeautifulPicture():
    ...
    def get_pic(self):
        ...
        print('开始获取所有a标签')
        all_a = BeautifulSoup(r.text, 'lxml').find_all('img', class_='_2zEKz')  #获取网页中的class为cV68d的所有a标签
        print('开始创建文件夹')
        ...
        for a in all_a: #循环每个标签，获取标签中图片的url并且进行网络请求，最后保存图片
            ...
            img_url = img_str #使用Python的切片功能截取双引号之间的内容
            #获取高度和宽度的字符在字符串中的位置
            width_pos = img_url.index('&w=')
            height_pos = img_url.index('&q=')
            width_height_str = img_url[width_pos : ] #使用切片功能截取高度和宽度参数，后面用来将该参数替换掉
